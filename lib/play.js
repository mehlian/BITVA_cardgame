...
function stealCard(gso, card){
	console.log("We are in stealCard")
	let attackingP = gso.players[gso.activePlayer];
	let defendingP = gso.players[Math.abs(gso.activePlayer-1)];
...
function exchangeCards(gso, card){
	let attackingP = gso.players[gso.activePlayer];
	let defendingP = gso.players[Math.abs(gso.activePlayer-1)];
	console.log(defendingP.deck.length);
	console.log(defendingP.deck.length);

	// Save a card to played
	
	if(defendingP.deck.length > 0 && defendingP.cards.length > 2 ){
		let i = 1;
		if (attackingP.deck>5){
			i = 5;
		} else {
			i = attackingP.deck;
		}
		...
		defendingP.cards = defendingP.deck.slice(0,i);

		defendingP.deck = defendingP.deck.slice(i)
