...
function hit(gso, card1, card2){
	if(!isActionPossible(gso)) {
		...
		if(attackCard.effect=="Hit"){
			...
			// check defenting player's item
			if(defendingP.item != null){
				// Don't forget to add a check that they both alive before returning
				return checkActions(effect(gso, attackCard, card2));
			} else {
				// Don't forget to add a check that they both alive before returning
...
function effect(gso, attackCard, card2){
	// set players
	let attackingP = gso.players[gso.activePlayer];
	let defendingP = gso.players[Math.abs(gso.activePlayer-1)];

	switch (defendingP.item.effect){
